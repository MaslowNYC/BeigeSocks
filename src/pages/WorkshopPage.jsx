...
function WorkshopPage() {
  ...
  return (
    <>
      <Helmet>
        <title>Workshop - Component Inventory | Beige Socks Workshop</title>
        <meta name="description" content="Browse and manage electronics components in Patrick's workshop inventory." />
      </Helmet>

      <div className="space-y-8">
        <div className="flex items-center justify-between">
          <h1 className="text-4xl font-bold text-[#F5F0E6]">Component Inventory</h1>
          <Button onClick={handleAddComponent} className="bg-[#D2B48C] text-[#0A0A0A] hover:bg-[#C4A67C]">
          ...
          {filters.map((filter) => (
            <button
              ...
              className={`px-4 py-2 rounded-lg font-medium transition-colors ${
                activeFilter === filter
                  ? 'bg-[#D2B48C] text-[#0A0A0A]'
                  : 'bg-[#1A1A1A] text-[#F5F0E6] hover:bg-[#2A2A2A]'
              }`}
            >
              {filter}
            </button>
          ...
          {filteredComponents.map((component, index) => (
            ...
            >
              <div className="flex items-start justify-between mb-4">
                <div>
                  <h3 className="text-xl font-bold text-[#F5F0E6] mb-1">{component.name}</h3>
                  <p className="text-[#64748B] text-sm">{component.category}</p>
                </div>
              ...
              <div className="space-y-2">
                <div className="flex items-center justify-between">
                  ...
                  <span className="text-[#F5F0E6] font-semibold">{component.quantity}</span>
                </div>
                <div>
                  <span className="text-[#64748B] text-sm block mb-1">Specs:</span>
                  <p className="text-[#F5F0E6] text-sm">{component.specs}</p>
                </div>
              </div>
            </motion.div>
          ))}
        ...
        {filteredComponents.length === 0 && (
          <div className="text-center py-12">
            <p className="text-[#64748B] text-lg">No components found matching your criteria.</p>
          </div>
        )}
      </div>
    </>
